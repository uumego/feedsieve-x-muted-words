// ==UserScript==
// @name         X Muted Words Manager (English Pack)
// @namespace    x-muted-words-manager-en
// @version      4.1.1
...
function status(s){let e=document.getElementById(P+'-status');if(e)e.innerText=s;console.log('[XMWM]',s)}function already(x){let s=new Set;for(let w of x.values())for(let k of keys(w))s.add(k);for(let w of success())for(let k of keys(w))s.add(k);return s}
...
function manager(){document.getElementById(P+'-mgr')?.remove();let o=document.createElement('div');o.id=P+'-mgr';o.style='position:fixed;inset:0;background:#000b;z-index:10000000;display:flex;align-items:center;justify-content:center';o.innerHTML=`<div style="width:min(820px,94vw);height:min(780px,90vh);background:#111;color:#fff;border-radius:16px;padding:16px;display:flex;flex-direction:column"><div style="display:flex;justify-content:space-between"><b>${T.mgr}</b><button id="${P}-close">${T.close}</button></div><input id="${P}-search" placeholder="${T.search}" style="margin:10px 0;padding:8px;background:#000;color:#fff;border:1px solid #444"><div style="display:flex;gap:8px"><textarea id="${P}-addtext" placeholder="${T.customph}" style="flex:1;height:65px;background:#000;color:#fff"></textarea><button id="${P}-add">${T.add}</button></div><div style="margin:8px 0"><button id="${P}-export">${T.export}</button> <button id="${P}-reset">${T.reset}</button></div><div id="${P}-stats" style="font-size:12px;color:#aaa"></div><div id="${P}-list" style="flex:1;overflow:auto"></div></div>`;document.body.appendChild(o);function render(){let q=n(document.getElementById(P+'-search').value),rows=W.map(w=>({w,src:'b',on:state(w),d:D.has(n(w))})).concat(custom().map(w=>({w,src:'c',on:true,d:false}))).filter(r=>!q||n(r.w).includes(q));document.getElementById(P+'-stats').textContent=`Built-in ${W.length} · default off ${D.size} · custom ${custom().length} · active ${active().length}`;document.getElementById(P+'-list').innerHTML=rows.map(r=>`<div style="display:flex;padding:6px;border-bottom:1px solid #222;opacity:${r.on?1:.45}"><span style="flex:1">${eh(r.w)} <small>${r.src==='c'?T.custom:(r.d?T.defaultoff:T.defaulton)}</small></span><button data-w="${eh(r.w)}" data-s="${r.src}">${r.src==='c'?T.delete:(r.on?T.disable:T.enable)}</button></div>`).join('');document.querySelectorAll('#'+P+'-list button').forEach(b=>b.onclick=()=>{let w=b.dataset.w;b.dataset.s==='c'?ss('custom',custom().filter(x=>n(x)!==n(w))):toggle(w,!state(w));render();draw()})}document.getElementById(P+'-close').onclick=()=>o.remove();document.getElementById(P+'-search').oninput=render;document.getElementById(P+'-add').onclick=()=>{let e=document.getElementById(P+'-addtext'),a=e.value.split(/[\n,，;；]+/).map(cl).filter(Boolean);ss('custom',uniq([...custom(),...a]));e.value='';render();draw()};document.getElementById(P+'-export').onclick=()=>{let a=document.createElement('a'),u=URL.createObjectURL(new Blob([active().join('\n')],{type:'text/plain;charset=utf-8'}));a.href=u;a.download='x-muted-en-'+S+'.txt';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000)};document.getElementById(P+'-reset').onclick=()=>{if(confirm(T.resetq)){del('off');del('on');render();draw()}};render()}
...
function panel(){if(document.getElementById(P+'-panel')||!document.body)return;let e=document.createElement('div');e.id=P+'-panel';e.style='position:fixed;top:12px;right:12px;z-index:9999999;width:320px;background:#111;color:#fff;padding:14px;border-radius:14px;font-family:Arial,sans-serif;box-shadow:0 8px 30px #0006';e.innerHTML=`<b>X Muted Words v${V}</b><div id="${P}-meta" style="font-size:11px;color:#888;margin:4px 0"></div><div id="${P}-status" style="white-space:pre-line;background:#181818;padding:10px;border-radius:8px;font-size:12px;margin:8px 0">${T.ready}</div><label>${T.next} <input id="${P}-int" type="number" min="${C.min}" max="${C.max}" style="width:80px"> ${T.sec}</label><div style="display:flex;gap:6px;margin-top:10px"><button id="${P}-start">${T.start}</button><button id="${P}-pause">${T.pause}</button></div><button id="${P}-once" style="width:100%;margin-top:6px">${T.once}</button><div style="display:flex;gap:6px;margin-top:6px"><button id="${P}-manage">${T.manage}</button><button id="${P}-scan">${T.scanbtn}</button></div><button id="${P}-local" style="width:100%;margin-top:6px">${T.local}</button>`;document.body.appendChild(e);document.getElementById(P+'-start').onclick=start;document.getElementById(P+'-pause').onclick=pause;document.getElementById(P+'-once').onclick=()=>batch(false);document.getElementById(P+'-manage').onclick=manager;document.getElementById(P+'-scan').onclick=async()=>{if(busy)return;busy=true;try{let m=await scan(status);status('Scanned: '+m.size)}finally{busy=false;draw()}};document.getElementById(P+'-local').onclick=()=>status(`Success: ${success().length} · Failed: ${(gs('bad',[])||[]).length} · Active: ${active().length}`);let i=document.getElementById(P+'-int');i.value=geti();i.onchange=()=>i.value=seti(i.value);draw()}
panel();setInterval(panel,1500);setInterval(draw,1000);window.XMutedWordsManager={version:V,active,scan,start,pause,manager,setIntervalSeconds:seti,builtin:()=>[...W]};
})();
